  ...
  <div class="feature-preview" :class="`feature-preview--${kind}`">
    ...
    <div v-else-if="kind === 'preview'" class="preview-demo">
      ...
      <div class="preview-demo__dialog">
        <img class="preview-demo__image" :src="demoImages[0].src" alt="Woman in green preview" />
    ...
    <div v-else-if="kind === 'spotlight'" class="spotlight-demo">
      <div class="spotlight-demo__shortcut">
        <span>Open from anywhere</span><KbdChip :shortcut="shortcut" size="md" />
      </div>
        ...
        <div class="spotlight-demo__surface">
          ...
          <div class="spotlight-demo__results">
            <article v-for="(image, index) in demoImages.slice(1, 4)" :key="image.fileName" :class="{ active: index === 0 }">
              <img class="demo-image" :src="image.src" :alt="image.label" />
              <div><strong>{{ image.fileName }}</strong><small>{{ 92 - index * 6 }}% match</small></div>
              <div class="spotlight-demo__actions">
                <Button variant="secondary" size="sm">Copy <KbdChip shortcut="Ctrl+C" size="sm" /></Button>
                <Button variant="secondary" size="sm">Explorer <KbdChip shortcut="Ctrl+E" size="sm" /></Button>
                <Button variant="primary" size="sm">Imagyx <KbdChip shortcut="Ctrl+I" size="sm" /></Button>
              </div>
            </article>
          </div>
        </div>
...
.mini-app { display: grid; grid-template-columns: 150px minmax(0, 1fr); height: 100%; min-height: 0; }
...
.demo-image { display: block; width: 100%; height: 100%; object-fit: cover; background: var(--skeleton); }
.sidebar-demo { display: flex; flex-direction: column; width: min(330px, calc(100% - 42px)); height: 100%; margin: 0 auto; padding: 22px 18px; border-inline: 1px solid var(--border); background: color-mix(in srgb, var(--sidebar) 96%, transparent); }
...
.search-demo { height: 100%; padding: 22px; }
.search-demo__header { display: grid; grid-template-columns: minmax(0, 1fr) auto; align-items: center; gap: 14px; }
...
.search-demo__input strong { color: var(--text-muted); font-size: 9px; }
.shortcut-hint { display: flex; align-items: center; gap: 7px; color: var(--text-muted); font-size: 9px; white-space: nowrap; }
.search-demo__grid { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: 10px; margin-top: 18px; }
.search-card { min-width: 0; padding: 6px; border: 1px solid transparent; border-radius: 12px; background: var(--surface); box-shadow: 0 12px 22px -22px rgb(15 23 42 / 0.5); }
...
.demo-image-shell { position: relative; display: block; width: 100%; aspect-ratio: 1.45; overflow: hidden; border-radius: 9px; }
.tag-strip { position: absolute; right: 0; bottom: 0; left: 0; display: flex; gap: 5px; padding: 17px 6px 6px; overflow: hidden; background: linear-gradient(transparent, rgb(2 6 23 / 0.78)); }
...
.preview-demo { position: relative; height: 100%; display: grid; place-items: center; padding: 26px; }
...
.preview-demo__image { grid-column: 1 / -1; width: 100%; height: 205px; object-fit: contain; border-radius: 13px; background: var(--background); }
...
.preview-demo__hints { position: absolute; z-index: 2; bottom: 12px; display: flex; align-items: center; gap: 14px; color: var(--text-muted); font-size: 9px; }
.preview-demo__hints span { display: flex; align-items: center; gap: 7px; }
.spotlight-demo { height: 100%; padding: 18px 22px 22px; }
.spotlight-demo__shortcut { display: flex; align-items: center; justify-content: flex-end; gap: 8px; min-height: 28px; color: var(--text-muted); font-size: 9px; }
...
.spotlight-demo__results .demo-image { width: 44px; height: 44px; border-radius: 9px; }
...
.settings-demo { display: grid; align-content: center; gap: 13px; height: 100%; padding: 22px; }
.settings-demo__shortcut-summary { display: flex; align-items: center; justify-content: space-between; gap: 14px; padding: 12px 14px; border: 1px solid var(--border); border-radius: 14px; background: var(--surface); }
.settings-demo__shortcut-summary > div { display: flex; align-items: center; gap: 10px; }
.settings-demo__shortcut-summary span { display: grid; }
.settings-demo__shortcut-summary strong { font-size: 11px; }
.settings-demo__shortcut-summary small { margin-top: 4px; color: var(--text-muted); font-size: 9px; }
...
.indexing-demo { display: grid; align-content: center; gap: 16px; width: min(510px, calc(100% - 52px)); height: 100%; margin: 0 auto; }
...
@media (max-width: 760px) {
  ...
  .spotlight-demo__actions { display: none; }
}
@media (prefers-reduced-motion: reduce) { .progress-track i { animation-duration: 0.01ms; } }
</style>
