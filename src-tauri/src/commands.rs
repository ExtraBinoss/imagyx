...
use crate::{
    ...
    ml,
    models::{
        AppInfo, FollowedFolder, ImageAsset, ImageEmbedding, ImageExplanation,
        ModelDownloadProgress, QueryConcept, RuntimeStats, SearchRequest, SemanticMatch,
    },
    state::AppState,
...
#[tauri::command]
pub fn update_runtime_stats(stats: RuntimeStats, state: State<'_, Arc<AppState>>) {
    *state.runtime_stats.write() = stats;
}
...
pub async fn prepare_ai_images(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        let wanted: std::collections::HashSet<&str> =
            image_ids.iter().map(String::as_str).collect();
        let by_id: HashMap<String, ImageAsset> = state
            .database
            .images(None)
            .map_err(|error| error.to_string())?
            .into_iter()
            .filter(|image| wanted.contains(image.id.as_str()))
...
pub fn explain_results(
    ...
    let wanted: std::collections::HashSet<&str> = image_ids.iter().map(String::as_str).collect();
    state
        .vectors
        .read()
        .par_iter()
        .filter(|entry| wanted.contains(entry.image_id.as_str()))
        .map(|entry| {
