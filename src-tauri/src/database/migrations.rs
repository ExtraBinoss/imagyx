...
pub(super) fn migrate(database: &Database) -> Result<(), AppError> {
    ...
    // Old builds used a generic model label and older prompt/preprocessing
    // profiles. Do not mix those vectors with the current search profile:
    // pending images will be regenerated by the normal resumable indexer.
