...
function remember(key: string, url: string): string {
  urls.delete(key)
  urls.set(key, url)
  while (urls.size > MEMORY_CACHE_MAX_ITEMS) {
...
async function runTask(task: QueueTask): Promise<void> {
  ...
  try {
    ...
    const url = remember(task.key, imagyxApi.fileUrl(path))
    perfSample('Thumbnail', 'IPC path and generation', performance.now() - startedAt)
    settle(task, url)
  } catch (error) {
